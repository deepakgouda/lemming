...
func TestBGP(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.desc, func(t *testing.T) {
			...
			bgpPath := ocpath.Root().NetworkInstance(fakedevice.DefaultNetworkInstance).
				Protocol(oc.PolicyTypes_INSTALL_PROTOCOL_TYPE_BGP, "BGP").Bgp()
			locRibQuery := bgpPath.Rib().AfiSafi(oc.BgpTypes_AFI_SAFI_TYPE_IPV4_UNICAST).Ipv4Unicast().LocRib().State()
			...
			// Get peer session state information
			for neighbor := range tc.dut.bgpOC.Neighbor {
				sessionStateQuery := bgpPath.Neighbor(neighbor).SessionState().State()
				if _, ok := gnmi.Watch(t, dut, sessionStateQuery, time.Second*30, func(val *ygnmi.Value[oc.E_Bgp_Neighbor_SessionState]) bool {
					state, ok := val.Val()
					t.Logf("~~~~~~BGP neighbor %s session state: %s~~~~~~", neighbor, state.String())
					return ok && state == oc.Bgp_Neighbor_SessionState_ESTABLISHED
				}).Await(t); !ok {
					t.Fatalf("~~~~~~BGP session for neighbor %s not established~~~~~~", neighbor)
				}
			}
